...
import { ArrowLeft, FileText, GripVertical, Trash2, Eye, EyeOff } from 'lucide-react'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-8">
              ...
              <div className="space-y-2">
                {Object.entries(FIELD_TYPES).map(([type, { label, icon }]) => (
                  <button
                    key={type}
                  ...
                  >
                    {icon && React.createElement(icon, { className: "h-5 w-5 text-gray-500" })}
                    <span>{label}</span>
                  </button>
                ))}
              </div>
